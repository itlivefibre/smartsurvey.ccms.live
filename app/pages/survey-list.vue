...
const {
  ...
  async () => {
    const res = await $fetch<{
      ...
      body: { accesskey, accessForm, project_id: projectId, user_id: userId },
    });
...
const filteredData = computed(() => {
  ...
  if (wardFilter.value.trim())
    items = items.filter((i) => i.ward_no === wardFilter.value.trim());
  if (poleFilter.value.trim())
    items = items.filter((i) => i.pole_no === poleFilter.value.trim());
  if (wattFilter.value.trim())
    items = items.filter((i) => i.watt === wattFilter.value.trim());
...
const totalWorking = computed(
  () => filteredData.value.filter((i) => i.light_status === "Working").length,
);
const totalNotWorking = computed(
  () =>
    filteredData.value.filter((i) => i.light_status === "Not Working").length,
);
const totalNA = computed(
  () => filteredData.value.filter((i) => i.light_status === "N/A").length,
);
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return h("img", {
        src: imageUrl,
        class:
          "w-10 h-10 object-cover rounded cursor-zoom-in border border-gray-200",
        loading: "lazy",
        onClick: () => {
...
<template>
  ...
  <div v-else>
        ...
        <div>
          <div class="w-full bg-emerald-100 rounded-full h-1 mt-1">
            <div
              ...
              :style="{
                width: ((totalWorking / totalSurveys) * 100 || 0) + '%',
              }"
            ></div>
          </div>
        ...
        <div>
          <div class="w-full bg-rose-100 rounded-full h-1 mt-1">
            <div
              ...
              :style="{
                width: ((totalNotWorking / totalSurveys) * 100 || 0) + '%',
              }"
            ></div>
          </div>
    ...
    <div>
      ...
      >
        <div
          class="bg-white p-4 rounded shadow-lg max-w-[90vw] max-h-[90vh] overflow-auto"
        >
