...
const repairmanId = ref<number | null>(null);
    ...
    $fetch(
      "https://ayodhya.water.live/api/surveyNew.php?action=get_repairing_list",
      {
        ...
        body: {
          ...
          repairman_id: repairmanId.value || "",
        },
      },
    ),
  {
    watch: [currentPage, pageSize, repairmanId],
  },
);

const { data: repairmanList } = await useAsyncData<{ result: Repairman[] }>(
  "repairmanList",
  () =>
    $fetch(
      "https://ayodhya.water.live/api/surveyNew.php?action=get_repairman_list",
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: {
  ...
  //   {
  //     watch: [currentPage, pageSize, debouncedRepairmanId],
  //   },
);
const repairmanOptions = computed(() => {
  return (
    repairmanList.value?.result.map((r) => ({
      label: `${r.name}`,
      value: r.repairman_id,
    })) || []
  );
});
...
const columns: ColumnDef<RepairRecord>[] = [
  ...
  //   { accessorKey: "user_id", header: "User Id" },
  {
    ...
    header: "Repairman",
    cell: ({ row }) => {
      const repairman = repairmanList.value?.result.find(
        (r) => r.repairman_id === row.original.user_id,
      );
      return repairman ? repairman.name : `ID: ${row.original.user_id}`;
    },
  },
...
// watch(repairmanId, (val) => {
//   clearTimeout(debounceTimer);

//   debounceTimer = setTimeout(() => {
//     debouncedRepairmanId.value = val;
//     currentPage.value = 1;
//   }, 50);
// });
</script>
    ...
    <div v-else>
      ...
      <UCard class="w-full p-4 ring-2 mb-10 border shadow-sm light">
        <div class="flex items-center gap-3 mb-3">
          <USelect
            v-model="repairmanId"
            :items="repairmanOptions"
            placeholder="Select Repairman"
            searchable
            class="w-64"
          />

          <UButton variant="outline" @click="repairmanId = null">
            Reset
          </UButton>
        </div>
