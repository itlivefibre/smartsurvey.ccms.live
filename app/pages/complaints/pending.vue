...
const resolutionStatusFilter = ref<"All" | "Resolved" | "Pending">("All");
...
const {
  ...
  result: {
    panding_0_12: Survey[];
    panding_12_24: Survey[];
    panding_1_2_days: Survey[];
    panding_2_30_days: Survey[];
    panding_30_plus: Survey[];
  };
}>(
  "complaints_report",
  () => {
    // ⛔ prevent invalid API calls
    // if (!accesskey.value) return null;
    // console.log(fromDate.value);
    // console.log(toDate.value);
    // if (
    //   (fromDate.value && toDate.value != null) ||
    //   (fromDate.value != null && toDate.value) ||
    //   (fromDate.value == null && toDate.value == null)
    // )
    // {
    // alert("input feild cant be empty");
...
const mergedData = computed(() => {
  ...
  const groups = [
    { key: "panding_0_12", label: "0–12 Hours" },
    { key: "panding_12_24", label: "12–24 Hours" },
    { key: "panding_1_2_days", label: "1–2 Days" },
    { key: "panding_2_30_days", label: "2–30 Days" },
    { key: "panding_30_plus", label: "30+ Days" },
  ];
...
onMounted(async () => {
  try {
    const res = await $fetch(
      "https://ayodhya.water.live/api/surveyNew.php?action=get_created_by",
      {
        method: "POST",
        body: { accesskey },
      },
...
const filteredData = computed(() => {
  ...
  } else if (resolutionStatusFilter.value === "Pending") {
    items = items.filter((i) => !i.resolve_image);
  }
...
const columns: ColumnDef<Survey>[] = [
  ...
  },
  {
    ...
    header: "Created By",
    cell: ({ row }) => {
      const userId = Number(row.original.user_id);
      const electrician = electricians.value.find((e) => e.value === userId);
...

  {
    ...
    cell: ({ row }) => {
      ...
      // Optional animated pulse for Pending
    ...
    <div v-else>
      ...
      <UCard class="p-4 ring-2 border">
        <div class="flex justify-between items-center mb-4">
          <h3 class="text-sm font-medium">Pending Complaint</h3>
          ...
          <div class="flex items-center gap-2">
            ...
            <!-- <USelect v-model="pageSize" :items="pageSizeOptions" class="w-18" /> -->
            <USelect
              v-model="pageSize"
              :items="
