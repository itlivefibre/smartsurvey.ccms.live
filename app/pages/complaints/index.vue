...
const {
  ...
  { lazy: true, watch: [fromDate, toDate] },
);
...
const username = computed(() => complaintData?.value?.user_name ?? []);
...
const filteredData = computed(() => {
  ...
  //   );
  if (poleOrMobileFilter.value.trim()) {
    const search = poleOrMobileFilter.value.toLowerCase();

    items = items.filter(
      (i) =>
        i.pole_no?.toLowerCase().includes(search) ||
        String(i.mobile_no ?? "").includes(search) ||
        String(i.ward_no ?? "").includes(search) ||
        String(i.id ?? "").includes(search) ||
        i.name?.toLowerCase().includes(search) ||
        i.reference_by?.toLowerCase().includes(search),
    );
  }
...
const columns: ColumnDef<Survey>[] = [
  ...
  },
  {
    ...
    header: "After Repair",
    cell: ({ row }) => {
      const imageFileName = row.getValue("resolve_image") as string | null;
      const imageUrl = baseImageUrl_thmb + imageFileName;
      const imageUrl_act = baseImageUrl + imageFileName;
      if (!imageFileName)
        return h(
...
function exportCSV() {
  ...
  const rows = filteredData.value.map((item) => {
    ...
    const resolveAt = item.resolve_date
      ? new Date(item.resolve_date).toLocaleString()
      : "N/A";
    // console.log("resolve_at-", resolveAt);
      ...
      <UCard class="w-full p-4 ring-2 mb-10 border shadow-sm light">
        <div class="flex justify-between items-center mb-2">
          ...
          <div class="flex items-center gap-2">
            ...
            <span
              class="text-sm font-semibold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-md"
            >
              {{ username }}
            </span>
          </div>
