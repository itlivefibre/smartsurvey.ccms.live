...
const onSubmit = async () => {
  ...
  try {
    const { data, error } = await useFetch(
      `https://ayodhya.water.live/api/surveyNew.php?action=applogin`,
      {
        method: "POST",
        body: {
    ...
    var survey_url = "/surveys";

    if (data.value?.result.role === "3") {
      var dashboard_url = "/dashboard_com_complaint";
      var survey_url = "/dashboard_com_survey";
    }
    setToken(
      data.value.result.access_key,
      data.value.result,
      dashboard_url,
      survey_url
    );
    ...
    router.push(dashboard_url); // Navigate after login
  } catch (err) {
    toast.add({
      title: "Error",
