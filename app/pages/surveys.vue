...
const complaintSeries = computed(() => [
  {
    name: "Survey",
    data: surveyUsers.value.map((u) => u.survey_count ?? 0),
  },
]);
...
async function fetchSurveys() {
  ...
  try {
    ...
    totalSurveys.value = res.result?.survey_records_count?.total_surveys ?? 0;
    surveyWorking.value = Number(
      res.result?.survey_records_count?.working_count ?? 0
    );
    surveyNotWorking.value = Number(
      res.result?.survey_records_count?.not_working_count ?? 0
    );
...
async function fetchComplaints() {
  ...
  try {
    ...
    totalComplaints.value =
      res.result?.complaint_records_count?.total_complaints ?? 0;

    complaintWorking.value = Number(
      res.result?.complaint_records_count?.working_count ?? 0
    );
    complaintNotWorking.value = Number(
      res.result?.complaint_records_count?.not_working_count ?? 0
    );
    complaintNA.value = Number(
      res.result?.complaint_records_count?.na_count ?? 0
    );
...
const chartOptions = computed(() => ({
  chart: {
    ...
    events: {
      dataPointSelection: function (
        ...
        const user = surveyUsers.value[index];
        ...
        // example actions
        // router.push(`/complaints/user/${user.user_id}`)
        // or show modal
      },
    },
  },
  ...
  xaxis: {
    categories: surveyUsers.value.map((u) => u.username),
    labels: {
      rotate: -45,
    },
  },
  tooltip: {
    y: {
      formatter: (val: number) => `${val} complaints`,
    },
    enabled: true,
    onDatasetHover: {
...
<template>
  <div class="p-6 bg-slate-50 min-h-screen">
    <div class="grid grid-cols-1 md:grid-cols-1 gap-6">
      ...
      <!-- Total Surveys -->
      <!-- Total Surveys Card -->
      <NuxtLink to="/survey-list" class="block">
        <div
        ...
        >
          <div class="flex items-center space-x-4">
            <!-- Icon -->
            <div
              class="p-4 bg-gradient-to-br from-indigo-100 to-indigo-200 rounded-full text-2xl"
            >
              📊
            </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-1 gap-6 mt-8">
      ...
      >
        <div class="flex items-center flex-wrap gap-2 mb-4">
          ...
          <span
            v-if="surveyDate"
            class="text-xs bg-indigo-50 text-indigo-700 px-3 py-1 rounded-full ml-2"
          >
        ...
        >
          <table class="min-w-full border-collapse text-sm">
            <thead
              class="bg-gradient-to-r from-indigo-50 to-teal-50 sticky top-0 z-10"
            >
              <tr>
